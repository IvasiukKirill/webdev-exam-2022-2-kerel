...
class Book(db.Model):
    ...
    def score(self):
        sum = 0
        comment = Comment.query.filter(self.id == Comment.id_book).all()
        for comment in comment:
            sum += comment.mark
        try:
            return sum / len(self.comment)
...
class Comment(db.Model):
    ...
    book = db.relationship('Book',overlaps="comment")
...
class Covers(db.Model):
    ...
    book = db.relationship('Book',overlaps="covers")
