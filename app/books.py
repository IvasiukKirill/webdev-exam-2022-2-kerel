...
from models import Book, Comment, Genry, Covers
...
def show(book_id):
    ...
    comment = Comment.query.filter(Comment.id_book == book_id).order_by(Comment.date_added.desc())
    curr_comment = None
    if current_user.is_authenticated:
        curr_comment = Comment.query.filter(Comment.id_book == book_id).filter(Comment.id_users == current_user.id).first()
    return render_template('book/show.html', book=book, image=image, curr_comment=curr_comment, comment=comment)


@book_bp.route('/<int:book_id>/comment')
@login_required
def comment(book_id):
    return render_template('book/comment.html', id_book=book_id)


@book_bp.route('/<int:book_id>/comment/create', methods=['POST'])
@login_required
def create_comment(book_id):
    new_comment = Comment()
    new_comment.id_book=book_id
    new_comment.id_users=current_user.id
    new_comment.mark=request.form.get('rating')
    new_comment.text=clean(request.form.get('text'))
    try:
        ...
        return redirect(url_for('books.comment', book_id=book_id))
